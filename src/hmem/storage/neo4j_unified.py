...
Graph Schema:
    Nodes:
        ...
        (:Fact {id, entity_id, slot, value, cardinality, embedding, is_superseded, created_at, updated_at, source_role, q_value, q_update_count})
        (:Process {id, trigger, action, outcome, context, problem_statement, key_insight, embedding, is_deprecated, created_at, updated_at, q_value, q_update_count})
        (:Principle {id, content, evidence_count, embedding, created_at, q_value, q_update_count, is_deprecated, version})
...
class Neo4jUnifiedStore:
    ...
    def find_conversation_by_session_id(self, session_id: str) -> dict | None:
        """Find a conversation by session_id.
        ...
        with self.driver.session(database=self.database) as session:
            result = session.run(
                ...
                RETURN c.id AS id, c.session_id AS session_id, c.created_at AS created_at
                LIMIT 1
                """,
    ...
    def add_process(self, process: Process, source_conv_id: str) -> str:
        ...
        Note: Deduplication should be handled by the extraction agent BEFORE
        calling this method. The agent should search for similar processes
        and decide whether to update existing or create new.
        ...
        with self.driver.session(database=self.database) as session:
            session.run(
                ...
                key_insight=process.key_insight,
    ...
    def search(self, query: str, limit: int = 10) -> list[Memory]:
        ...
        This is a convenience method that wraps fulltext_search.
        Used to satisfy Neo4jStoreProtocol.
        ...
        Returns:
            List of Memory objects
        """
        return self.fulltext_search(query=query, node_types=None, limit=limit)
    ...
    def _node_to_memory(self, node: Any, score: float, node_type: str) -> Memory | None:
        ...
        try:
            ...
            if node_type == "Fact":
                ...
                key_insight = node_props.get("key_insight", "")
                content = f"When {trigger} → {action}"
                if key_insight:
                    content += f" (Insight: {key_insight})"
