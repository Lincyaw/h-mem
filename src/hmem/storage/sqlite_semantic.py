...
class SQLiteSemanticStore:
    ...
    def resolve_conflict(
        self, subject: str, predicate: str, old_object: str, new_object: str
    ) -> int:
        ...
        with self.SessionLocal() as session:
            ...
            new_triple = SemanticTriple(
                subject=subject, predicate=predicate, object=new_object, weight=1.0
            )
    ...
    def query_related(
        ...
        with self.SessionLocal() as session:
            results = (
                session.query(SemanticFactRow)
                .filter(SemanticFactRow.subject == entity)
                .all()
            )
    ...
    def prune_low_weight(self, threshold: float = 0.3) -> int:
        ...
        with self.SessionLocal() as session:
            deleted = (
                session.query(SemanticFactRow)
                .filter(SemanticFactRow.weight < threshold)
                .delete()
            )
            session.commit()
            return deleted
