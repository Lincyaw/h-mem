...
class MultiScaleReflectionPolicy(ReflectionPolicy):
    ...
    def __init__(
        ...
        if not (immediate_threshold < daily_threshold < weekly_threshold):
            raise ValueError("Thresholds must be increasing: immediate < daily < weekly")
    ...
    def should_trigger(
        ...
        if episode_count >= self.thresholds["immediate"] and \
           time_span_days < 1 and \
        ...
        if episode_count >= self.thresholds["daily"] and \
           1 <= time_span_days < 7:
            logger.info(
                "reflection_triggered",
                level="daily",
        ...
        if episode_count >= self.thresholds["weekly"] and \
           time_span_days >= 7:
            logger.info(
                "reflection_triggered",
                level="weekly",
