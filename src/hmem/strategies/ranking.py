...
by relevance, combining multiple signals (similarity, recency, importance).
"""
...
class HybridRanker(RetrievalRanker):
    """Hybrid ranking combining similarity, recency, and importance.

    Default weights based on information retrieval best practices:
    - similarity_weight: 0.6 (primary signal)
    - recency_weight: 0.2 (temporal decay)
    - importance_weight: 0.2 (access frequency)
    ...
    def __init__(
        ...
        Args:
            similarity_weight: Weight for similarity score (0.5-0.7 recommended)
            recency_weight: Weight for temporal recency (0.1-0.3 recommended)
            importance_weight: Weight for access importance (0.1-0.3 recommended)
        ...
        """
        total_weight = similarity_weight + recency_weight + importance_weight
