...
def main():
    ...
    conversation1 = Conversation(
        ...
        messages=[
            ...
            Message(
                role="assistant", content="Great! Are you using requests or selenium?"
            ),
            Message(
                role="user", content="I tried requests but it failed on dynamic sites"
            ),
    ...
    print(
        f"   ✓ Speedup: {first_time / cached_time if cached_time > 0 else float('inf'):.1f}x faster"
    )
